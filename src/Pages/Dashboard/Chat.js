...
function Chat(props) {
  ...
  const [showMoreMessage, setShowMoreMessage] = useState(
    <>
      <CachedIcon sx={{ marginRight: "5px" }} /> {"Show Previous Messages"}
    </>
  );
        ...
        (async () => {
          ...
          try {
            const response = await req(
              api + "/api/message/get/" + params.id + "?limit=" + messageLimit
            );
            ...
            if (result.success) {
              if (
                messageBody.current &&
                JSON.stringify(messages) !== JSON.stringify(result.data)
              ) {
                setSendingFile("");
                scrollToBottom();
              }
                ...
                >
                  ...
                  >
                    <Badge
                      ...
                      sx={{
                        "& .MuiBadge-badge": {
                          ...
                          mr: 0.4,
                          bgcolor:
                            visitor.status == "offline" ? "#EFA20B" : "#1B8700",
                        },
                      }}
                      anchorOrigin={{
                        vertical: "bottom",
                    ...
                    <div className="flex">
                      ...
                      <span>
                        {visitor?.message?.message !== undefined ? (
                          visitor?.message?.isMine == true ? (
                            <span>
                              <b>Me:</b>{" "}
                              {visitor?.message?.message?.substring(0, 20)}
                            </span>
                          ) : (
                            <span>
                              {visitor?.message?.message?.substring(0, 20)}
                    ...
                    <div className="flex go-right">
                      <span className="chatTime">
                        {visitor?.message?.time == undefined
                          ? ""
                          : moment(visitor?.message?.time).format("hh:mm a")}
                      </span>
                      {visitor.message && (
                        <span className="tick">
                          {visitor?.message?.seen == "true" ? (
                            <DoneAllIcon
                              color="success"
                              sx={{ fontSize: "10px" }}
                            />
                          ) : (
                            <DoneIcon sx={{ fontSize: "10px" }} />
                          )}
                        </span>
                      )}
                    </div>
  ...
  const sendMessage = () => {
    ...
    if (token) {
      if (params.id) {
        (async () => {
          try {
            const response = await req(api + "/api/message/send/" + params.id, {
              method: "POST",
              body: JSON.stringify({
                message,
              }),
          ...
          if (xhr.upload) {
            xhr.upload.onprogress = function (e) {
              let done = e.position || e.loaded,
                total = e.totalSize || e.total;
              setFileProgess(
                Math.floor(Math.floor((done / total) * 1000) / 10)
              );
            };
          }
  ...
  const sendImage = () => {
    ...
    file.onchange = async () => {
      if (file.files[0]) {
        ...
        try {
          let response = await fetch(api + "/api/upload/" + params.id, {
            method: "POST",
            body: formData,
  ...
  useEffect(() => {
    if (message.trim().length == 0) {
      ...
      setActionButton(<SendIcon sx={{ fontSize: 30 }} onClick={sendMessage} />);
    }
  }, [message]);
                      ...
                      {visitor.message && (
                        <div className="flex go-right">
                          <span className="chatTime">
                            {visitor?.message?.time == undefined
                              ? ""
                              : moment(visitor?.message?.time).format(
                                  "hh:mm a"
                                )}
                          </span>
                          <span className="tick">
        ...
        <div className="page main">
          <div className="chatHead">
            <div style={{display: "flex", paddingTop: "1rem"}}>
              ...
              <Badge
                ...
                className={
                  visitor.status == "offline" ? "badgeOffline" : "badgeOnline"
                }
              >
                <Avatar
                  className="visitor-img"
              ...
              <div id="activeStatusChat">
                <h3>
                  <Link to={"/edit/" + id}>{visitor.name}</Link>
                </h3>
                <font
                  style={{}}
                  color={
                ...
                >
                  {visitor?.status ? visitor.status : "unknown"}
                </font>
              </div>
              <div className="chat-head-actions" style={{display: "flex"}}>
                <MoreVertIcon />
            ...
            <div
              ...
              style={{
                  display: isBotControlVisibile ? "block" : "none",
                  placeSelf: "center"
              }}
            >
              <FormGroup>
          ...
          <div id="chatBody" ref={messageBody}>
            ...
            {messages.length == undefined ? (
              <div className="no_messages">Loading Messages ...</div>
            ) : messages.length == 0 ? (
          ...
          <div id="coverTextBox">
            <div style={{ display: "flex" }} className="p-1" id="textbox">
          ...
          >
            ...
            >
              <center className="actionButton">
                ...
                >
                  <AttachFileIcon sx={{ fontSize: 30 }} onClick={attachFile} />
                </Avatar>
                <b>File</b>
              </center>
              <center className="actionButton">
                <Avatar
                  ...
                  style={{ height: "50px", width: "50px" }}
                >
                  <InsertPhotoIcon sx={{ fontSize: 30 }} onClick={sendImage} />
                </Avatar>
                <b>Image</b>
              </center>
              <center className="actionButton">
          ...
          <Snackbar onClose={handleClose} open={isOpen} autoHideDuration={5000}>
            <MuiAlert
              onClose={handleClose}
              elevation={6}
              variant="filled"
